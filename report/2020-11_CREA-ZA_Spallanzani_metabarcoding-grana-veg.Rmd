...
* Eukaryota (%): fraction of reads, for each sample, classified as Eukaryota
...
OTU_milk$merged = merge(x=OTU_milk$meta,   y=OTU_milk$OTU_rel_screen, by.x='Label_IGA', by.y='row.names', all = TRUE)
OTU_milk$merged = merge(x=OTU_milk$merged, y=OTU_milk$OTU,            by.x='Label_IGA', by.y='row.names', all = TRUE)
OTU_milk$merged$reads = rowSums(OTU_milk$merged[, 10:11], na.rm = TRUE)
OTU_milk$merged = OTU_milk$merged[,-10:-11]

#renaming cols, for clarity
colnames(OTU_milk$merged) = c('Label_IGA', 'Id_Lab', 'type', 'caseificio', 'MESE', 'data_prelievo', 'province', 'Eukaryota (%)', 'UN (%)', 'reads')

#sorting by interesting col
OTU_milk$merged = OTU_milk$merged[order(OTU_milk$merged[,9], decreasing = TRUE),]
...
## Clade/Phylum
```{r, fig.height = 8, fig.width = 8, fig.align = "center", echo=FALSE, include=TRUE}
level = 'clade_phylum'
OTU_milk = build_OTU_table(raw_OTUs, level) 
...
#do heatmap
h = prepare_heatmap(OTU_milk$OTU_rel_noUN, min_abundance=0)
...
#do heatmap
h = prepare_heatmap(OTU_milk$OTU_rel)
