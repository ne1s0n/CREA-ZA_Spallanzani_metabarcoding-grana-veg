...
Beta diversity is a way to quantify the differences between two communities. There are several possible metrics available, the two more commonly used being *Bray–Curtis* beta diversity and *Weighted Unifrac* beta diversity. [**CITATION NEEDED**]. Given that it consists of a series of pair-wise comparisons between samples, beta diversity takes naturally the shape of a triangular matrix. An helpful way to represent it post Multi Dimensional Scaling, so that the two most meaningful dimensions are plotted.
...
alpha_long = melt(alpha, id.vars = c('sample', 'type', 'level', 'province'), variable.name = 'alpha')
```
...
cur = rarecurve(OTU_milk$OTU, step = 1, col = "blue", label = FALSE, cex = 0.6)
abline(v=min_reads)
```

The plot clearly shows that all samples reached stability.

## Beta diversity

[TODO]
```{r compute beta, echo=FALSE, include=FALSE, eval=FALSE}
#single level
