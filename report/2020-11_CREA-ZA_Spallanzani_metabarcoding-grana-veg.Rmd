...
p = ggplot(alpha_long, aes(x=alpha, y=value, fill=type)) +
#p = ggplot(subset(alpha_long, type == 'GP area'), aes(x=alpha, y=value, fill=type)) 
 geom_boxplot() +  facet_wrap(~level, nrow = 1) + 
...
p = ggplot(subset(tmp, type == 'GP milk'), aes(x=value, fill=alpha)) + 
...
#saving the figure
ggsave(filename = paste(sep='', figure_outfolder, 'diversity_alpha_density.tiff'), plot = p, device = 'tiff', width = 1800, height = 1575, units = 'px')
